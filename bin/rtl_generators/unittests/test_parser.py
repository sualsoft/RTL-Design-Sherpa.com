...
class ParserTest(unittest.TestCase):
    def test_parserhelper_processSignalFreeform0(self):
        ...
        self.assertEqual(tstList, [], f'Issue with processSignalFreeform: {tstStr}')
    ...
    def test_parserhelper_processSignalFreeform1(self):
        ...
        self.assertEqual(
            tstList,
            tstProcessSigFree1,
            f'Issue with processSignalFreeform: {tstStr}',
        )
...
#        pprint.pprint(tstList)
    ...
    def test_parserhelper_processSignal1(self):
        ...
        self.assertEqual(tstRec, tstProcessSig1, f'Issue with processSignal: {tstStr}')

    def test_parserhelper_processSignal2(self):
        tstStr = 'strVal[15]'
        tstRec = ParserHelper.processSignal(tstStr, 'input', 'logic')
        self.assertEqual(tstRec, tstProcessSig2, f'Issue with processSignal: {tstStr}')

    def test_parserhelper_processSignal3(self):
        tstStr = 'strVal2[15:0][7:4]'
        tstRec = ParserHelper.processSignal(tstStr, 'output', 'logic')
        self.assertEqual(tstRec, tstProcessSig3, f'Issue with processSignal: {tstStr}')
    ...
    def test_parserhelper_array_merge1_advanced(self):
        ...
        print(f"Advanced Array Merge 1: {arrMerge}")
        self.assertEqual(
            arrMerge,
            mrgVal,
            f'Array Merge issue, expected: {mrgVal} received: {arrMerge}',
        )
        arrA = '[(DW-1):0][7:0]'
        arrB = '[(DW-1):0][15:8]'
        mrgVal = '[(DW-1):0][15:0]'
        arrMerge = ParserHelper.arrayMerge(arrA, arrB)
        print(f"Advanced Array Merge 2: {arrMerge}")
        self.assertEqual(
            arrMerge,
            mrgVal,
            f'Array Merge issue, expected: {mrgVal} received: {arrMerge}',
        ...
        arrMerge = ParserHelper.arrayMerge(arrA, arrB)
        print(f"Advanced Array Merge 3: {arrMerge}")
        self.assertEqual(
            arrMerge,
            mrgVal,
            f'Array Merge issue, expected: {mrgVal} received: {arrMerge}',
